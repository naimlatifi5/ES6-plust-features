export const promises = () => {
  ...
  function multiplePromisesInOne() {
    ...
    Promise.all([doSomething, doSomethingElse, oneMorePromise]).then((response) => {
  ...
  function printName(string, callback) {
    setTimeout(() => {
      console.log('print name randomly', string);
      callback();
    }, Math.floor(Math.random() * 100) + 1);
  }

  // function printAllNames() {
  //   console.log("Are we printing names in the same order ? ")
  //   printName("George")
  //   printName("Alice")
  //   printName("Oscar")
  // }
  ...
  // ES5
  function printAllNamesCallbackHell() {
    console.log('Are we printing names with the same order');
    printName('George', () => {
      printName('Alice', () => {
        printName('Oscar', () => {});
      });
  ...
  console.log('============Print all names callback hell===========');
  printAllNamesCallbackHell();

  // ES6
  console.log('========== ES6 =================');
  function printNameES6(string) {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        console.log('print name randomly ES6', string);
        resolve();
      }, Math.floor(Math.random() * 100) + 1);
    });
  }

  function printAllNamesES6() {
    printNameES6('George')
      .then(() => {
        return printNameES6('Alice');
      })
      .then(() => {
        return printNameES6('Oscar');
      });
  }
